...
public class BoardController {
	...
	public String page_list(HttpServletRequest request, Model model) {
	...
	public String modify_view(HttpServletRequest request, Model model, HttpSession session) {
		...
		String sid = (String) session.getAttribute("sessionId");
		if(sid == null) { //로그인하지 않은 상태
			model.addAttribute("msg", "로그인한 회원만 수정 가능합니다.");
			model.addAttribute("url", "login");
			
			return "alert/alert";
		} else {
			String bnum = request.getParameter("bnum"); //유저가 클릭한 글의 번호
			// 수정하기 전 글들을 보여주게 하는 코드
			BoardDao boardDao = sqlSession.getMapper(BoardDao.class);
			BoardDto boardDto = boardDao.contentViewDao(bnum);
	...
	public String boardmodify(HttpServletRequest request, Model model) {
		...
		if(result == 1) { //수정 성공->1, 실패->0
			...
			model.addAttribute("url", "page_list");			
		} else {
	...
	public String boarddelete(HttpServletRequest request, Model model, HttpSession session) {
		...
		String sid = (String) session.getAttribute("sessionId");
		if(sid == null) { //로그인하지 않은 상태
			model.addAttribute("msg", "로그인한 회원만 삭제 가능합니다.");
			model.addAttribute("url", "login");
